# -*- coding: utf-8 -*-
"""
Created on Fri Jun  4 18:55:07 2021
...
data = pd.read_excel("middle_cut.xls")
fe_cr_ni=data.iloc[:,:3]
delta_strain=data.iloc[:,3:]

train_data = fe_cr_ni[:700]
val_data = fe_cr_ni[700:886]
test_data = fe_cr_ni[886:]

# evaluate model with standardized dataset
train_target1 =np.array(delta_strain[:700])
val_target1 = np.array(delta_strain[700:886])
test_target1 = np.array(delta_strain[886:])
...
history = model_1.fit(train_data, train_target, epochs=500, batch_size=35, shuffle=True, validation_data=(val_data, val_target), verbose=2)
...
plt.xticks(np.linspace(0.85 , 1.2 , 8))
plt.yticks(np.linspace(0.85 , 1.2 , 8))
plt.subplot(122)
...
plt.xticks(np.linspace(0.009 , 0.013 , 5))
plt.yticks(np.linspace(0.009 , 0.013 , 5))
plt.show()
